...
Insight.prototype.import = function(pk,name, cb) {
  preconditions.checkArgument(pk);
  preconditions.checkArgument(name);
  preconditions.shouldBeFunction(cb);

  this.requestPost('/api/addr/import', {
    pk: pk,
    name: name
...
Insight.prototype.sendfrom = function(from,to,amount, cb) {
  preconditions.checkArgument(from);
  preconditions.checkArgument(to);
  preconditions.checkArgument(amount);
  preconditions.shouldBeFunction(cb);

  this.requestPost('/api/tx/sendfrom', {
    from: from,
    to: to,
    amount: amount,
  }, function(err, res, body) {
